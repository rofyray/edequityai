/**
 * Seed script: Populate modules + curriculum_content tables with 6-8 curriculum data.
 *
 * Run: npx tsx scripts/seed-band2.ts
 *
 * Idempotent: deletes existing 6-8 MVP data before inserting.
 * Generates OpenAI embeddings for all curriculum chunks.
 */

import { createAdminClient } from './lib/supabase-admin'
import { BAND2_CURRICULUM } from '../src/lib/ingestion/curriculum-catalog'
...
async function main() {
  console.log('🌱 EdEquity AI — Band 2 (6-8) Curriculum Seed Script')
  console.log(`📚 Modules to seed: ${BAND2_CURRICULUM.length}`)
  console.log('')

  const supabase = createAdminClient()

  // Step 1: Clean existing 6-8 MVP data
  console.log('🗑️  Cleaning existing 6-8 data...')
  ...
  const moduleRows = BAND2_CURRICULUM.map((m) => ({
    slug: m.slug,
    title: m.title,
    ...
    xp_reward: 60, // Slightly higher for middle school
    content: m.content,
  ...
  console.log('')
  console.log('═══════════════════════════════════════')
  console.log('  Band 2 (6-8) Seed complete!')
